window.addEventListener('load', function () {
  ...
  var box = document.getElementById('home-login');
  ...
  var authListener = firebase.auth().onAuthStateChanged(function (user) {
    ...
    flushStoredErrors();
    sendAcqusition().then(handleLoggedIn).catch(function (err) {
...
var initAuthBox = function initAuthBox() {
  var getStartedBtn = document.getElementById('get-started');
  document.getElementById("login--login-box").addEventListener("click", function () {
    history.pushState('signup', null, window.location.pathname); // redirect(`${window.location.pathname}index.html?signup=1`)
